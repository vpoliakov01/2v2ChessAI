...
export function NumberInput(
	{ value, onChange, min = 0, max = Infinity, step = 1, editable = false, disableButtons = false, width = 28 }:
		NumberInputProps,
) {
	...
	return (
		<div className={styles.numberInputContainer}>
			<div className={styles.numberInputControls}>
				{!disableButtons && (
					<button className={styles.numberInputButton} onClick={decrement} disabled={value <= min}>-</button>
				)}
				<input
					type='text'
					value={editValue}
				...
				/>
				{!disableButtons && (
					<button className={styles.numberInputButton} onClick={increment} disabled={value >= max}>+</button>
				)}
			</div>
		</div>
	);
